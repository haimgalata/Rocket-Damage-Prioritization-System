...
def distance_to_closest_hospital(lat: float, lon: float, deadline: float | None = None):
    ...
    for radius in search_radii:
        if deadline is not None and time.monotonic() >= deadline:
            logger.warning(f"[GIS:hospital] Deadline reached before radius={radius}m for ({lat},{lon})")
            break
        try:
